...
def generator_sample(split_data, landmark_data, image_path, saving_path, face_size):
    ...
    for i in range(len(split_data)):
        ...
        while True:
            ...
            '(1)主要用于生成正样本和部分样本'
            # if img_w / w > 1.5 * w:
            #     ratio1 = np.random.randint(-w * 0.06, w * 0.06)  # 中心点移动的范围
            #     ratio2 = np.random.randint(h * 0.06, h * 0.06)
            # else:
            #     ratio1 = np.random.randint(-(img_w - w) * 0.15, (img_w - w) * 0.15)  # 中心点移动的范围
            #     ratio2 = np.random.randint(-(img_h - h) * 0.15, (img_h - h) * 0.15)
            # side_ = np.random.randint(int(min(w, h) * 0.68), int(1.25 * min(w, h)))
            # cx_ = cx + ratio1
            # cy_ = cy + ratio2
            #
            # x1_ = np.maximum(cx_ - side_ / 2, 0)  # 如果减的结果小于0，超过图片了，就取0
            # y1_ = np.maximum(cy_ - side_ / 2, 0)  # 如果超过左边，就取0
            # x2_ = x1_ + side_  # 如果超过的左边的图片，就取图片的边长
            # y2_ = y1_ + side_
            ...
            if iou_p >= 0.7 and label_scale_num['positive'] > 0:
                ...
                if face_size == 48:
                    positive_anno_file.write(
                        f"positive/{positive_count}.jpg {1} {offset_x1} {offset_y1} {offset_x2} {offset_y2} {offset_px1} {offset_py1} {offset_px2} {offset_py2} {offset_px3} {offset_py3} {offset_px4} {offset_py4} {offset_px5} {offset_py5}\n")
                ...
                if label_scale_num['positive'] % 3 == 0:
                    # 随机增强
                    face_resize = enhance.rand_choice_enhance(face_resize, np.random.randint(0, 6))
                ...
                if face_size == 48:
                    part_anno_file.write(
                        f"part/{part_count}.jpg {2} {offset_x1} {offset_y1} {offset_x2} {offset_y2} {offset_px1} {offset_py1} {offset_px2} {offset_py2} {offset_px3} {offset_py3} {offset_px4} {offset_py4} {offset_px5} {offset_py5}\n")
