...
class Train:
    ...
    def __init__(self, root, batch_size=10, index=0, epoch=10):
        ...
        self._optimizer = torch.optim.SGD(self._net.parameters(), lr=1e-3)
    ...
    def __call__(self):
        ...
        for epoch in range(self._EPOCH):
            ...
            print(f"{epoch + self._index + 1} | train_acc：{np.mean(train_acc_list_inter)} | train_loss: {np.mean(train_loss_list_inter)}")
            ...
            print(f"{epoch + self._index + 1} | val_acc  ：{np.mean(val_acc_list_inter)} | val_loss  : {np.mean(val_loss_list_inter)}")
    ...
    def _draw_acc_loss_figure(self):
        ...
        plt.subplot(122)
        plt.title("Loss")
        plt.plot(self._epoch_list, self._train_loss_list, c="orange", label="train")
        plt.plot(self._epoch_list, self._val_loss_list, label="val")
        plt.legend()
    ...
    def _summary_writer_acc_loss_etc(self, train_acc_list_inter, val_acc_list_inter, test_acc_list):
        if isinstance(self._summary_writer, str):
            with open(self._summary_writer, "a+") as f:
                f.write(
                    f"{self._save_params} {np.mean(train_acc_list_inter)} {np.mean(val_acc_list_inter)} {np.mean(test_acc_list)}")
                f.write("\n")
                f.flush()
...
if __name__ == '__main__':
    ...
    """
    13 | test_acc：0.7844444248411391
    19 | test_acc：0.8494444191455841 - 3层卷积，1层mlp，使用池化，加入增强
    24 | test_acc：0.8533333175712161
    31 | test_acc：0.8572222027513716
    36 | test_acc：0.8638888729943169
    
    14 | test_acc：0.8494444191455841
    32 | test_acc：0.8649999797344208
    40 | test_acc：0.8649999797344208
    
    
    51 | test_acc：0.874999980131785
    88 | test_acc：0.894999987549252
    """
