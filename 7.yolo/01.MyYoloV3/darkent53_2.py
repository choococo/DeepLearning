...
class ResidualLayer(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.sub_module = torch.nn.Sequential(
            ConvolutionalLayer(in_channels, in_channels * 2, 1, 1, 0, groups=in_channels // 2),  # 110，和311都不会改变特征图的大小
            ConvolutionalLayer(in_channels * 2, in_channels * 2, 3, 1, 1, groups=in_channels * 2),
            ShuffleBlock(groups=in_channels * 2),
            ConvolutionalLayer(in_channels * 2, in_channels, 1, 1, 0, groups=in_channels // 2)
        )
...
class ConvolutionalSets(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.sub_module = torch.nn.Sequential(
            ConvolutionalLayer(in_channels, out_channels, 1, 1, 0, groups=in_channels // 2),
            ShuffleBlock(groups=in_channels // 2),
            ConvolutionalLayer(out_channels, out_channels, 3, 1, 1, groups=out_channels),
            ConvolutionalLayer(out_channels, in_channels, 1, 1, 0, groups=in_channels // 2),
...
if __name__ == '__main__':
    ...
    """
    Total params: 26,178,670
    Trainable params: 26,178,670
    Non-trainable params: 0
    ----------------------------------------------------------------
    Input size (MB): 1.98
    Forward/backward pass size (MB): 3165.05
    Params size (MB): 99.86
    Estimated Total Size (MB): 3266.89
    """